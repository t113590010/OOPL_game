...
// ==========================================
void Queen::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
    // 確保有足夠的圖 (0~5，共 6 張)
    if (allFrames.size() < 6) return;

    std::vector<AnimFrame> walk, attack,idle;
    float offsetY = 0.0f;
    int walkcount = 6; // Index 0~8站立 從9~
    int atkcount =11;  // Index 3, 4, 5
    ...
    AnimFrame kbFrame;
    kbFrame.parts.push_back({allFrames[13], 0.0f, offsetY});
    ...
    SetAnimFPS(10.0f); // 基本貓可以走快一點，12 FPS 差不多
}