...
DogDoin::DogDoin(const Vector2& pos)
    : Entity(
        pos,
...
// ==========================================
void DogDoin::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
    // 確保有足夠的圖 (0~5，共 6 張)
    if (allFrames.size() < 6) return;

    std::vector<AnimFrame> walk, attack,idle;
    float offsetY = -100.0f;
    int walkcount = 4; // Index 0~8站立 從9~
