...
DinoCat::DinoCat(const Vector2& pos)
    : Entity(
        pos,
...
// ==========================================
void DinoCat::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
    // 確保有足夠的圖 (0~5，共 6 張)
    if (allFrames.size() < 6) return;

    std::vector<AnimFrame> walk, attack,idle;

    int walkcount = 4; // Index 0, 1, 2
    int atkcount =10;  // Index 3, 4, 5
