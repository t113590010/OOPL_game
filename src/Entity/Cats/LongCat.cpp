...
LongCat::LongCat(const Vector2& pos)
    : Entity(
        pos,
        UnitData::Get(UnitID::LongCat).hp,           // 血量
        UnitData::Get(UnitID::LongCat).speed,        // 跑速
        UnitData::Get(UnitID::LongCat).damage,       // 傷害
        UnitData::Get(UnitID::LongCat).attackRange,  // 攻擊距離
        UnitData::Get(UnitID::LongCat).attackCd,     // 攻擊冷卻
        UnitData::Get(UnitID::LongCat).cost,         // 價格
        UnitData::Get(UnitID::LongCat).spawnCd,      // 產兵冷卻
        UnitData::Get(UnitID::LongCat).rank,         // 階級
        UnitData::Get(UnitID::LongCat).imgPath,      // 圖片路徑
        UnitData::Get(UnitID::LongCat).kb            // 擊退次數
    ) {
    m_IsPlayerTeam = true;
    SetAoE(UnitData::Get(UnitID::LongCat).isAoE);
}
void LongCat::Update(float dt) {
    UpdateCooldown(dt);
...
// ==========================================
void LongCat::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
    // 確保有足夠的圖 (0~5，共 6 張)
