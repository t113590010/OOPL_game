#include "Entity/Enemies/Snack.hpp"
#include "Entity/UnitData.hpp"

Snack::Snack(const Vector2& pos)
    : Entity(
        pos,
        UnitData::Get(UnitID::Snack).hp,           // 150
        UnitData::Get(UnitID::Snack).speed,        // -40.0f (敵人向左走，速度應為負)
        UnitData::Get(UnitID::Snack).damage,       // 15
        UnitData::Get(UnitID::Snack).attackRange,  // 60.0f
        UnitData::Get(UnitID::Snack).attackCd,     // 1.5f
        UnitData::Get(UnitID::Snack).cost,         // 0
        UnitData::Get(UnitID::Snack).spawnCd,      // 0.0f
        UnitData::Get(UnitID::Snack).rank,         // 1
        UnitData::Get(UnitID::Snack).imgPath,      // 圖片路徑
        UnitData::Get(UnitID::Snack).kb            // 擊退次數
    ) {
    SetAoE(UnitData::Get(UnitID::Snack).isAoE);
...
// ==========================================
void Snack::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
