#include "Entity/Enemies/hippo.hpp"
#include "Entity/UnitData.hpp"

hippo::hippo(const Vector2& pos)
    : Entity(
        pos,
        UnitData::Get(UnitID::hippo).hp,           // 150
        UnitData::Get(UnitID::hippo).speed,        // -40.0f (敵人向左走，速度應為負)
        UnitData::Get(UnitID::hippo).damage,       // 15
        UnitData::Get(UnitID::hippo).attackRange,  // 60.0f
        UnitData::Get(UnitID::hippo).attackCd,     // 1.5f
        UnitData::Get(UnitID::hippo).cost,         // 0
        UnitData::Get(UnitID::hippo).spawnCd,      // 0.0f
        UnitData::Get(UnitID::hippo).rank,         // 1
        UnitData::Get(UnitID::hippo).imgPath,      // 圖片路徑
        UnitData::Get(UnitID::hippo).kb            // 擊退次數
    ) {
    SetAoE(UnitData::Get(UnitID::hippo).isAoE);
...
// ==========================================
void hippo::InitAnimation(const std::vector<SpriteFrame>& allFrames) {
    // 💡 安全檢查：確保有抓到 index 6 (共 7 張以上)
    if (allFrames.size() < 8) return;
