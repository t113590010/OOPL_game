...
namespace GameConfig {
    ...
    constexpr float MONEY_TEXT_X = 0.8f;
    constexpr float MONEY_TEXT_Y = 0.8f;

    constexpr float CAMERA_OFFSET = 500.0f;
    constexpr float CAMERA_MIN_X = PLAYER_BASE_X + CAMERA_OFFSET;
    constexpr float CAMERA_MAX_X = ENEMY_BASE_X - CAMERA_OFFSET;
    constexpr float CAMERA_SPEED = 800.0f;

    constexpr float SPAWN_OFFSET_X = 80.0f;

    // === 新增：單位尺寸設定 (寬度, 高度) ===
    constexpr float BASE_SIZE_X = 250.0f;  // 基地通常比較大
    constexpr float BASE_SIZE_Y = 250.0f;

    constexpr float CAT_SIZE_X = 100.0f;   // 貓咪的大小
    constexpr float CAT_SIZE_Y = 100.0f;

    constexpr float ENEMY_SIZE_X = 100.0f; // 敵人的大小
    constexpr float ENEMY_SIZE_Y = 100.0f;
